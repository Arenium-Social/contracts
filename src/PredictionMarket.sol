...
import {OracleInterfaces, OptimisticOracleConstraints} from "@uma/core/contracts/data-verification-mechanism/implementation/Constants.sol";
    ...
    Ownable
{
    ...
    mapping(bytes32 => PredictionMarketLib.AssertedMarket)
        private assertedMarkets; // Maps assertionId to AssertedMarket.
    ...
    event MarketAsserted(
        bytes32 indexed marketId,
        string indexed assertedOutcome,
    ...
    function initializeMarket(
        ...
        PredictionMarketLib.validateMarketParameters(
            outcome1,
            outcome2,
            description,
            markets[marketId]
    ...
    function assertMarket(
        ...
        uint256 minimumBond = optimisticOracle.getMinimumBond(
            address(currency)
        ); // optimisticOraclev3 might require higher bond.
        ...
        bytes memory claim = PredictionMarketLib.composeClaim(
            assertedOutcome,
            market.description,
            block.timestamp
        );
        ...
        assertedMarkets[assertionId] = PredictionMarketLib.AssertedMarket({
            asserter: msg.sender,
            marketId: marketId
        });
    ...
    function assertionResolvedCallback(
        ...
        PredictionMarketLib.Market storage market = markets[
            assertedMarkets[assertionId].marketId
        ];
    ...
    function createOutcomeTokens(
        ...
        PredictionMarketLib.createOutcomeTokens(
            market,
            msg.sender,
            tokensToCreate,
            currency
    ...
    function redeemOutcomeTokens(
        ...
        PredictionMarketLib.redeemOutcomeTokens(
            market,
            msg.sender,
            tokensToRedeem,
            currency
    ...
    function settleOutcomeTokens(
        ...
        payout = PredictionMarketLib.calculatePayout(
            market,
            outcome1Balance,
            outcome2Balance
        );
        ...
        )
    {
        PredictionMarketLib.Market storage market = markets[marketId];
        require(
            address(market.outcome1Token) != address(0),
            "Market does not exist"
        );

        (address token1, address token2) = PredictionMarketLib
            .getMarketTokenAddresses(market);
        (string memory out1, string memory out2) = PredictionMarketLib
            .getMarketOutcomes(market);
        (bool res, bytes32 assertId) = PredictionMarketLib.getMarketStatus(
            market
        );
    ...
    function getMarketTokens(
        bytes32 marketId
    ) external view returns (address outcome1Token, address outcome2Token) {
        PredictionMarketLib.Market storage market = markets[marketId];
        require(
            address(market.outcome1Token) != address(0),
            "Market does not exist"
    ...
    function getMarketOutcomes(
        bytes32 marketId
    ) external view returns (string memory outcome1, string memory outcome2) {
        PredictionMarketLib.Market storage market = markets[marketId];
        require(
            address(market.outcome1Token) != address(0),
            "Market does not exist"
    ...
    function getMarketStatus(
        bytes32 marketId
    ) external view returns (bool resolved, bytes32 assertedOutcomeId) {
        PredictionMarketLib.Market storage market = markets[marketId];
        require(
            address(market.outcome1Token) != address(0),
            "Market does not exist"
    ...
    function getAssertedMarket(
        bytes32 assertionId
    ) external view returns (address asserter, bytes32 marketId) {
        PredictionMarketLib.AssertedMarket
            memory assertedMarket = assertedMarkets[assertionId];
    ...
    function getOutcomeTokenBalances(
        bytes32 marketId,
        address account
        ...
        return (
            market.outcome1Token.balanceOf(account),
            market.outcome2Token.balanceOf(account)
        );
    }
}
